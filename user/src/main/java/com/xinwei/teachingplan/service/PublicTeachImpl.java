    ...
    public List<MenuEntity> menuQuery(){
        ...
        for (MenuEntity menu : menus) {
            if(menu !=null && menu.getFatherId() == -1){
                resultList.add(new MenuEntity(menu.getId(),menu.getName(),menu.getFatherId(),new ArrayList<>()));
            }else{
                tempList.add(new MenuEntity(menu.getId(),menu.getName(),menu.getFatherId(),new ArrayList<>()));
            }
        }
